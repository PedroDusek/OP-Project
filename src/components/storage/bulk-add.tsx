...
import { BULK_ADD_IDLE } from '@/app/(app)/binders/state'
import {
  countActiveFilters,
  toApiQuery,
  toCatalogQuery,
...
export function BulkAdd({
  ...
  const [state, submit, saving] = useActionState(bulkAddAction, BULK_ADD_IDLE)
  ...
  useEffect(() => {
    ...
    toast({
      ...
      description: `${state.cards} ${state.cards === 1 ? 'carta' : 'cartas'} em ${locationName}`,
      tone: 'success',
    })
  }, [state, locationName, toast])
    ...
    <div className="flex flex-col gap-4">
      ...
      */}
          ...
          <div className="flex items-center gap-3 rounded-card border border-border bg-surface p-3 shadow-sheet">
            <span className="flex min-w-0 flex-1 flex-col">
              ...
              <span className="truncate text-xs text-text-muted tabular-nums">
                {chosen.length} {chosen.length === 1 ? 'carta' : 'cartas'} · {locationName}
              </span>
            </span>
      ...
      <ConfirmDialog
        ...
        title={`Adicionar ${copies} ${copies === 1 ? 'cópia' : 'cópias'} a ${locationName}?`}
        description={`${chosen.length} ${chosen.length === 1 ? 'carta diferente' : 'cartas diferentes'}. As cópias entram na sua coleção e ficam guardadas neste local. Não dá para desfazer de uma vez.`}
